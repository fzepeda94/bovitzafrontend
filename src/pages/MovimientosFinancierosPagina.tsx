...
export function MovimientosFinancierosPagina() {
  ...
  const resetDistribucionCosto=()=>{setFormularioCosto(false);setAmbitoCosto("Animales");setMetodoCosto("Igualitaria");setBeneficiarioCosto("");setMontoCosto("");setInicioCosto("");setFinCosto("");setObservacionesCosto("");setAnimalesCosto({});setAnimalesResueltos([]);setVistaCosto(null);setBusquedaAnimalesCosto("");setPaginaAnimalesCosto(1)};
          ...
          {detalle.naturaleza === "GastoOperativo" && (
            <div className="mt-5 border-t pt-5">
              ...
              {formularioCosto&&<div className="mt-4 grid gap-4 rounded-xl border p-4 md:grid-cols-2">
                ...
                {ambitoCosto!=="Animales"&&<Select label="Beneficiario *" value={beneficiarioCosto} onChange={e=>{setBeneficiarioCosto(e.target.value);limpiarAnimalesDistribucion()}}><option value="">Seleccionar...</option>{(ambitoCosto==="Propietario"?entidades.data?.items:ambitoCosto==="LoteCompra"?lotes.data:ambitoCosto==="Finca"?fincas.data?.items:potreros.data?.items)?.map(x=><option key={x.id} value={x.id}>{"nombreCompletoORazonSocial" in x?x.nombreCompletoORazonSocial:(x.codigo??x.codigoAnimal??"")+" "+(x.nombre??"")}</option>)}</Select>}
