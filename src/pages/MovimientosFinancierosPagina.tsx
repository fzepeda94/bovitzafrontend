...
interface DistribucionCostoVista { id:string; ambito:string; beneficiarioId:string|null; beneficiarioNombre:string|null; metodo:string; montoDistribuir:number; fechaInicio:string|null; fechaFin:string|null; observaciones:string|null; asignaciones:AsignacionCostoVista[] }
...
export function MovimientosFinancierosPagina() {
  ...
  const [busquedaAnimalesCosto,setBusquedaAnimalesCosto]=useState(""); const [paginaAnimalesCosto,setPaginaAnimalesCosto]=useState(1);
  const [busquedaGruposCosto,setBusquedaGruposCosto]=useState(""); const [paginaGruposCosto,setPaginaGruposCosto]=useState(1);
  ...
  const animalesDisponiblesCosto=useQuery({queryKey:["animals","cost-distribution",busquedaAnimalesCosto,paginaAnimalesCosto],enabled:formularioCosto&&ambitoCosto==="Animales",queryFn:()=>api<PagedResult<Opcion>>(`/animales?page=${paginaAnimalesCosto}&pageSize=25&search=${encodeURIComponent(busquedaAnimalesCosto)}`)});
  const gruposCosto=useQuery({queryKey:["grupos-productivos","cost-distribution",busquedaGruposCosto,paginaGruposCosto],enabled:formularioCosto&&ambitoCosto==="GrupoProductivo",queryFn:()=>api<PagedResult<Opcion>>(`/grupos-productivos?estado=Activo&page=${paginaGruposCosto}&pageSize=20&search=${encodeURIComponent(busquedaGruposCosto)}`)});
  ...
  const resetDistribucionCosto=()=>{setFormularioCosto(false);setAmbitoCosto("Animales");setMetodoCosto("Igualitaria");setBeneficiarioCosto("");setMontoCosto("");setInicioCosto("");setFinCosto("");setObservacionesCosto("");setAnimalesCosto({});setAnimalesResueltos([]);setVistaCosto(null);setBusquedaAnimalesCosto("");setPaginaAnimalesCosto(1);setBusquedaGruposCosto("");setPaginaGruposCosto(1)};
  const limpiarAnimalesDistribucion=()=>{setAnimalesCosto({});setAnimalesResueltos([]);setVistaCosto(null)};
  const abrirDetalle=(x:Movimiento)=>{resetDistribucionCosto();setDetalle(x)};
  ...
  const guardarCosto=async()=>{if(!detalle||!vistaCosto)return;try{await api(`/finanzas/movimientos/${detalle.id}/distribuciones`,{method:"POST",body:JSON.stringify(solicitudCosto())});notify({tone:"success",title:"Costo asignado",message:"La distribución se guardó sin crear otro movimiento financiero."});resetDistribucionCosto();await client.invalidateQueries({queryKey:["financial-movement-costs",detalle.id]})}catch(error){notify({tone:"error",title:"No se asignó el costo",message:error instanceof Error?error.message:"Ocurrió un error."})}};
  const nombreBeneficiario=(x:DistribucionCostoVista)=>{if(!x.beneficiarioId)return "Selección explícita";if(x.ambito==="GrupoProductivo")return x.beneficiarioNombre??"Grupo productivo no disponible";if(x.ambito==="Propietario")return entidades.data?.items.find(e=>e.id===x.beneficiarioId)?.nombreCompletoORazonSocial??x.beneficiarioId;if(x.ambito==="LoteCompra")return lotes.data?.find(e=>e.id===x.beneficiarioId)?.nombre??x.beneficiarioId;if(x.ambito==="Finca")return fincas.data?.items.find(e=>e.id===x.beneficiarioId)?.nombre??x.beneficiarioId;return potreros.data?.items.find(e=>e.id===x.beneficiarioId)?.nombre??x.beneficiarioId};
      ...
      {detalle && (
        <Card className="mt-5">
          <div className="flex justify-between">
            ...
            <IconButton label="Cerrar detalle" onClick={() => {resetDistribucionCosto();setDetalle(null)}}>
              <X size={18} />
            </IconButton>
          </div>
          <p className="mt-3">
          ...
          {detalle.naturaleza === "GastoOperativo" && (
            <div className="mt-5 border-t pt-5">
              ...
              {!formularioCosto&&<Button className="mt-4" disabled={(costos.data?.montoPendienteAsignar??detalle.monto)<=0||detalle.estado==="Cancelado"} onClick={()=>{resetDistribucionCosto();setFormularioCosto(true);setMontoCosto(String(costos.data?.montoPendienteAsignar??detalle.monto))}}>Asignar costo</Button>}
              {formularioCosto&&<div className="mt-4 grid gap-4 rounded-xl border p-4 md:grid-cols-2">
                ...
                <Select label="Ámbito *" value={ambitoCosto} onChange={e=>{setAmbitoCosto(e.target.value);setBeneficiarioCosto("");limpiarAnimalesDistribucion()}}><option value="Animales">Animales</option><option value="Propietario">Propietario</option><option value="LoteCompra">Lote de compra</option><option value="Finca">Finca</option><option value="Potrero">Potrero</option><option value="GrupoProductivo">Grupo productivo</option></Select>
                ...
                {ambitoCosto==="GrupoProductivo"&&<div><Input label="Buscar grupo" value={busquedaGruposCosto} onChange={e=>{setBusquedaGruposCosto(e.target.value);setPaginaGruposCosto(1)}}/><Pagination page={paginaGruposCosto} totalPages={Math.max(1,Math.ceil((gruposCosto.data?.total??0)/20))} totalItems={gruposCosto.data?.total??0} pageSize={20} onPageChange={setPaginaGruposCosto} label="Grupos productivos"/></div>}
                {ambitoCosto!=="Animales"&&<Select label="Beneficiario *" value={beneficiarioCosto} onChange={e=>{setBeneficiarioCosto(e.target.value);limpiarAnimalesDistribucion()}}><option value="">Seleccionar...</option>{(ambitoCosto==="Propietario"?entidades.data?.items:ambitoCosto==="LoteCompra"?lotes.data:ambitoCosto==="Finca"?fincas.data?.items:ambitoCosto==="GrupoProductivo"?gruposCosto.data?.items:potreros.data?.items)?.map(x=><option key={x.id} value={x.id}>{"nombreCompletoORazonSocial" in x?x.nombreCompletoORazonSocial:(x.codigo??x.codigoAnimal??"")+" "+(x.nombre??"")}</option>)}</Select>}
                {(metodoCosto==="AnimalDias"||ambitoCosto==="Propietario"||ambitoCosto==="Finca"||ambitoCosto==="Potrero")&&<><Input label="Fecha inicio" type="date" value={inicioCosto} onChange={e=>{setInicioCosto(e.target.value);limpiarAnimalesDistribucion()}}/><Input label="Fecha fin" type="date" value={finCosto} onChange={e=>{setFinCosto(e.target.value);limpiarAnimalesDistribucion()}}/></>}
                ...
                <div className="flex gap-2 md:col-span-2"><Button onClick={()=>void previsualizarCosto()}>Vista previa</Button><Button variant="ghost" onClick={resetDistribucionCosto}>Cancelar</Button></div>
              </div>}
              ...
              {(costos.data?.distribuciones.length??0)>0&&<div className="mt-5"><h3 className="font-bold">Distribuciones registradas</h3><div className="mt-2 space-y-2">{costos.data?.distribuciones.map(x=><div key={x.id} className="flex items-center justify-between rounded-xl bg-slate-100 p-3 dark:bg-slate-800"><div><strong>{x.ambito} · {nombreBeneficiario(x)} · {x.metodo}</strong><span className="block text-sm text-slate-500">{x.asignaciones.length} animales · {formatearMoneda(x.montoDistribuir,moneda,cultura)}{x.fechaInicio&&x.fechaFin?` · ${x.fechaInicio.slice(0,10)} al ${x.fechaFin.slice(0,10)}`:""}</span></div><IconButton tone="danger" label="Quitar distribución" onClick={()=>void quitarDistribucion(x.id)}><Power size={17}/></IconButton></div>)}</div></div>}
            </div>
          )}
        </Card>
      )}
